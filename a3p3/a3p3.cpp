...
int main() {
	...
	for (int i = 0; i < map->getList().size(); i++)
	{
		...
		watcher->currentZone = zone;
		zone->attach(watcher);
	}

	cout << "Game Ready!" << endl;
	
	cout << endl << endl << "[3] Main Game Loop" << endl << endl;

	cout << endl << "Current Game Statistics:" << endl;
	for (int i = 0; i < map->getList().size(); i++)
	{
		GameMapNode* curZone = map->getList().at(i);
		watcher->currentZone = curZone;
		curZone->notify();
	}
	cout << endl;
	...
	cout << endl << "Final Game Stats:" << endl;
	for (int i = 0; i < map->getList().size(); i++)
	{
		GameMapNode* curZone = map->getList().at(i);
		watcher->currentZone = curZone;
